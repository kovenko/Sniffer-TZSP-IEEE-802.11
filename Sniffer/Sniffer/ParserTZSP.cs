...
namespace Sniffer
{
    ...
    public class ParserTzsp
    {
        ...
        public static void ToString(TzspPacket tzsp)
        {
            Console.WriteLine("Protocol TZSP parser");
            Console.WriteLine($"Reseived packet length {tzsp.packet.Length} bytes; {BitConverter.ToString(tzsp.packet)}");
            Console.WriteLine($"\tVersion: {tzsp.header.version};");
            Console.WriteLine($"\tType: {GetHeaderType(tzsp.header.type)};");
            Console.WriteLine($"\tEncapsulated protocol: {GetProtocol(tzsp.header.encapsulated_protocol)};");
            foreach(var item in tzsp.fields)
            {
                ...
                else
                {
                    ...
                    Console.WriteLine($"\tField: {item.Key}: {val};");
                }
            }
            Console.WriteLine($"\tEncapsulated_packet: {BitConverter.ToString(tzsp.encapsulated_packet)};");
            Console.WriteLine("");
        }
